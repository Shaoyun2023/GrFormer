...
def _split_image_to_patches(image, base_size=256):
    h, w = image.shape
    pad_h = (base_size - (h % base_size)) % base_size
    pad_w = (base_size - (w % base_size)) % base_size
    ...
    padded_h = h + pad_h
    padded_w = w + pad_w
    rows = padded_h // base_size
    cols = padded_w // base_size
    ...
    for r in range(rows):
        for c in range(cols):
            start_r = r * base_size
            start_c = c * base_size
            block = image_tensor[start_r:start_r + base_size, start_c:start_c + base_size]
            block = block.reshape(1, 1, base_size, base_size)
            images.append(block.float())
...
def get_test_image1(paths, height=None, width=None, flag=False):
    ...
    for path in paths:
        ...
        if h < base_size or w < base_size:
            images, meta = _split_image_to_patches(image, base_size=base_size)
            c = len(images)
            continue
...
def recons_fusion_images_any(img_lists, h, w, meta):
    img_f_list = []
    rows = meta["rows"]
    cols = meta["cols"]
    ...
    for i in range(len(img_lists[0])):
        block0 = img_lists[0][i]
        device = block0.device
        img_f = torch.zeros(1, pad_h, pad_w, device=device)

        for idx, block in enumerate(img_lists):
            r = idx // cols
            c = idx % cols
            start_r = r * base_size
            start_c = c * base_size
            img_f[:, start_r:start_r + base_size, start_c:start_c + base_size] = block[i]
