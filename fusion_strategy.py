...
def nuclear_pooling(tensor, kernel_size=None):
    shape = tensor.size()
    vectors = torch.zeros(1, shape[1], 1, 1).to(args.device)
...
#     spatial_w2 = spatial_w2.repeat(1, shape[1], 1, 1)
#
#     weight_min1 = torch.min(spatial_w1)
#     weight_max1 = torch.max(spatial_w1)
#     x1 = (spatial_w1 - weight_min1) / (weight_max1 - weight_min1)
#     weight_min2 = torch.min(spatial_w2)
#     weight_max2 = torch.max(spatial_w2)
#     x2 = (spatial_w2 - weight_min2) / (weight_max2 - weight_min2)
#
#     tensor_f =  spatial_w1 * tensor1 +spatial_w2 * tensor2
#
#     return tensor_f
